...
int main(int argc, char **argv)
{
    ...
    /* 
     * Check and print student info 
     */
    if (student_check) {
	/* Students must fill in their student information */
	if (!strcmp(student.id, "")) {
	    printf("ERROR: Please provide the information about your POVIS ID in mm.c.\n");
	    exit(1);
	} else if (!strcmp(student.id, "povisid")) {
	    printf("ERROR: Please provide the information about your POVIS ID in mm.c.\n");
	    exit(1);
	} else
	    printf("POVIS ID: %s\n", student.id);
	if ((*student.name == '\0') || (*student.email == '\0')) {
	    printf("ERROR.  You must fill in all the fields of student!\n");
	    exit(1);
	}
	else
